...
3. Hit **Start** once the search is done. Watch the per-video progress; a summary appears at the end. Videos that already have subtitles in a language are skipped, so re-running is always safe. That covers external files like `movie.en.srt`, `movie.eng.srt` or `movie.srt` with the language detected from the text, and subtitle tracks embedded in the video, which show as "embedded" in the table. Only the languages you selected are checked; forced-only tracks and tracks without a language tag are ignored on purpose, they are no substitute for a full subtitle. Embedded tracks can be ignored altogether via a setting if you want external files anyway. A checkbox below the table hides videos that already have everything.
